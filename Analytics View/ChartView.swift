...
struct ChartView: View
{
    ...
    func total() -> [totalData]
    {
        ...
        for kind in kinds
        {
            if kind == "need"
            {
                totalValue.append(totalData(name: kind, value: toDegree(value: totalWill(kind: "need")), color: Color.init(red: 82 / 255, green: 215 / 255, blue: 38 / 255)))
            }
            if kind == "want"
            {
                totalValue.append(totalData(name: kind, value: toDegree(value: totalWill(kind: "want")), color: Color.init(red: 255 / 255, green: 115 / 255, blue: 0 / 255)))
            }
            if kind == "credit"
            {
                totalValue.append(totalData(name: kind, value: toDegree(value: totalType(kind: "credit")), color: Color.init(red: 255 / 255, green: 0 / 255, blue: 0 / 255)))
            }
            if kind == "debit"
            {
                totalValue.append(totalData(name: kind, value: toDegree(value: totalType(kind: "debit")), color: Color.init(red: 0 / 255, green: 126 / 255, blue: 214 / 255)))
            }
        }
        return totalValue
    }
    ...
    struct DrawShape: View {
        ...
        func getDegreeFrom() -> Double
        {
            if index == 0 {
                ...
                for i in 0...index - 1
                {
                    temp += Double(listtotalData[i].value / 100) * 360
                }
